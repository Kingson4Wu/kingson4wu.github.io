...
- **Transformer**：根据当前上下文计算下一个 token 的 logits，例如“猫=5.2、狗=4.8、鱼=2.1”。
- **Softmax**：把这些 logits 转换成概率，例如“猫=55%、狗=37%、鱼=6%”。
- **激活函数**：只是 Transformer 内部神经网络计算的一部分，不负责决定最终选择哪个 token。
- **Greedy / Argmax**：直接选择概率最高的 token。比如“猫”的概率是 55%，就一定选择“猫”，结果基本确定。
- **Sampling**：按照概率随机抽取。例如“猫”占 55%、“狗”占 37%，这次可能抽到“猫”，下次也可能抽到“狗”，随机性就来自这里。
- **Temperature**：调整概率分布。低 temperature 会让概率更集中，结果更稳定；高 temperature 会让概率更分散，结果更随机。
- **Top-K / Top-P**：进一步限制随机抽样的候选范围。Top-K 只保留概率最高的 K 个 token；Top-P（Nucleus Sampling）则按照概率从高到低累加，只保留累计概率达到 P 的最小候选集合。简单说，Top-K 固定“最多几个候选”，Top-P 固定“覆盖多少概率”，候选数量会根据当前分布动态变化。

为什么最终差异会很大？因为第一个 token 一旦选择不同，后面的上下文就不同，模型重新计算出的概率也会不同，随机性会逐步放大。

所以核心是：模型本身主要是在做概率预测，真正引入随机性的主要是后面的 sampling / decoding 策略。
