...
[`labali-spotify-publish-episode`](https://github.com/Kingson4Wu/labali-skills) 做的事情并不复杂：在 Spotify for Creators 里发布播客 episode。它不用 API，只走浏览器；它默认复用人工登录态；它既要能进入发布流程，也要能判断什么时候发布真的完成了。
