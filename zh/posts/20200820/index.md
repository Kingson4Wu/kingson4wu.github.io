...
Canonical: https://kingson4wu.github.io/zh/posts/20200820/
Markdown: https://kingson4wu.github.io/zh/posts/20200820/index.md
Language: zh
Type: post
Date: 2020-08-20
Tags: Networking

CDN缓存的几点总结 CDN资源的标识: url 跟HTTP缓存无关的请求头不影响CDN缓存（亲测） 不仅是静态资源，接口的数据可以做CDN缓存（亲测） CDN服务一般都有默认的缓存配置（比如一分钟），服务端可以通过设置HTTP缓存相关的Header控制是否适用CDN缓存 HTTP缓存和CDN缓存分别作为客户端缓存和服...
