...
以下内容基本都是从书籍中摘抄，用于个人记录 从AI得出的回答不一定正确或者只是现阶段暂时正确 增加部分个人理解 图：LLM position 图：LLM LLM（Large Language Model） 大语言模型的涌现能力被非形式化定义为“在小型模型中不存在但在大模型中出现的能力”，具体是指当模型扩展到一定规模时，...
