...
Canonical: https://kingson4wu.github.io/zh/posts/20260128-softmax/
Markdown: https://kingson4wu.github.io/zh/posts/20260128-softmax/index.md
Language: zh
Type: post
Date: 2026-01-28
Tags: AI, LLM, Deep Learning, Math

以下内容由AI辅助生成 Softmax 是多分类任务中最常见的输出层函数。它的任务表面上是“把 logits 变成概率”，但本质是：将一组可加的分数转换为可比较、可优化的相对强度，并与极大似然/交叉熵无缝对接，同时保证数值稳定和梯度友好。 1. 问题设定：从 logits 到概率 1.1 什么是 logits？ 神经网...
...
- 其他 logit 都是负数：$e^{\text{负数}}$ 最多下溢到 0，不会 NaN
...
若用其他正函数 $g(x)$ 替代指数，梯度会出现复杂的 $g^{\prime}(x)/g(x)$ 项，优化困难且不稳定。
...
**例子**：
- 如果 $p_{\text{猫}} = 0.7, p_{\text{狗}} = 0.2$
- 比值：$p_{\text{猫}}/p_{\text{狗}} = 3.5$，表示“猫的可能性是狗的 3.5 倍”
